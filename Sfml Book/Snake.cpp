...
Direction Snake::GetPhysicalDirection(){
	...
	if (head.position.x == neck.position.x){
		return (head.position.y > neck.position.x) ? Direction::Down : Direction::Up;
	}
...
void Snake::UpdateLeaderBoards(){
	std::ifstream leaderBoardsFileIn;
	leaderBoardsFileIn.open("leader_boards.txt");
	std::string prevScores = "";
	std::string lines;
	while (std::getline(leaderBoardsFileIn, lines))
	{
		prevScores = prevScores + lines + "\n";
	}
	std::string score = prevScores + std::to_string(GetScore()) + "\n";
	leaderBoardsFileIn.close();

	std::ofstream leaderBoardsFileOut;
	leaderBoardsFileOut.open("leader_boards.txt");
	leaderBoardsFileOut << score;
